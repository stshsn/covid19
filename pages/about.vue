<template>
  <div class="About">
    ...
    <StaticCard>
      {{
        $t(
          '当サイトは新型コロナウイルス感染症 (COVID-19) に関する最新情報を提供するために、野村弘樹（福井高専卒, '
          )
      }}
      <a :href="$t('https://no4.co.jp/')" target="_blank" > {{ $t('株式会社ナンバーフォー') }} </a> 入社）が開設したものです。
      <br /><br />
      {{
        $t(
          '複製・改変が許されたオープンソースライセンスで公開されている、 東京都公式新型コロナウイルス対策サイト の 仕組み を利用しています。'
        )
      }}<br />
      <br />
      {{
    ...
    <StaticCard>
      ...
      <p>{{
        $t(
          '本サイトに関するお問い合わせは、以下のメールアドレスへお願いいたします。'
          )
      }}</p>
      <br />
      <a href="mailto: covid19.fukui@gmail.com">covid19.fukui@gmail.com</a>
    </StaticCard>
    <StaticCard>
      <h3>{{ $t('ブラウザ環境について') }}</h3>
      <p>
    ...
    <StaticCard>
      ...
      <p>
        {{
          $t(
            '当サイトに掲載されている情報の正確性については万全を期していますが、開発者は利用者が当サイトの情報を用いて行う一切の行為について責任を負うものではありません。'
          )
        }}
      </p>
      <p>
